...
# 第三步（质量增强）：当前 AI CLI 生成 outline.yaml 后，回传给脚本做分节素材召回和新版 prompt
python3 wechat-article-pipeline/scripts/framework_flow.py \
  --channel tech \
  --topic "我把公众号发布系统重构成 channel 模型" \
  --goal click --goal save \
...
# 第二步（降级）：跳过素材库召回，brief 中 auto_materials 为空
python3 wechat-article-pipeline/scripts/framework_flow.py \
  --channel tech \
  --topic "我把公众号发布系统重构成 channel 模型" \
  --goal click --goal save \
...
# 第三步：如需直接生成初稿，追加 --generate-draft
python3 wechat-article-pipeline/scripts/framework_flow.py \
  --channel tech \
  --topic "我把公众号发布系统重构成 channel 模型" \
  --goal click --goal save \
...
- `wechat-article-pipeline/work/framework-flow/*.yaml`：brief
...
5. **大纲准备（质量增强，推荐）**：选框架后优先跑 `framework_flow.py --code N --prepare-outline`。脚本只生成 `*.outline.prompt.md`，不调用 LLM；在哪个 AI CLI 会话发起写作任务，就由当前会话读取这个 prompt 并生成 `*.outline.yaml`。
   - **当前会话生成大纲**：Codex / Claude Code / Hermes 等当前 CLI 负责生成结构化大纲，脚本不写死具体模型。
   - **大纲文件回传**：生成 `outline.yaml` 后，用 `framework_flow.py --code N --outline-file <path>` 回传给脚本。脚本会校验 section 数量、标题、core_viewpoint、supporting_angles 等字段。
...
6. **分节素材召回（大纲后、动笔前）**：传入 `--outline-file` 后，脚本根据每节的 `core_viewpoint + title + topic anchor + supporting_angles` 做分节召回，每节最多挂 1-2 条可选参考，并全局去重。召回结果只注入 `primary_claim`，不注入原文正文。
...
7. 判断文章类型：
   ...
   - 观点文
   - 情绪 + 观点混合文
8. 明确目标读者、情绪按钮、传播方向（必须与人设配置中的读者画像一致）
9. 先写正文初稿（必须以人设配置中的叙述者身份来写，将分节召回的素材自然嵌入正文）
10. 写完后自动执行一次"轻度降 AI + 增加人味"的后处理
11. 做违禁词排查（使用 `banned-word-guard`），发现违禁词自动替换为安全表达
12. 根据 `channel + framework + goal + 内容特征` 推荐排版主题（Top 3）
13. 最后补齐发布素材：
   - 封面风格选择（必须让用户从 huashu / warm-gradient / accent-bar / minimal / geometric 中选择）
   - 标题备选（必须符合人设配置中的标题风格）
   - 摘要
   - 导语
   - 封面主标题 / 副标题
